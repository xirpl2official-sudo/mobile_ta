...
btnSimpan.text = "Perbarui"
    ...
    private fun showRowPopupMenu(anchor: View, siswa: SiswaItem) {
        ...
        popup.menu.add(0, 4, 2, "Ubah Status")
        popup.menu.add(0, 3, 3, "Hapus")
    ...
    private fun showUpdateStatusDialog(siswa: SiswaItem) {
        ...
        MaterialAlertDialogBuilder(this)
            .setTitle("Ubah Status - ${siswa.nama_siswa}")
            .setSingleChoiceItems(statusOptions.map { statusDisplay[it] ?: it }.toTypedArray(), selectedIndex) { dialog, which ->
                val newStatus = statusOptions[which]
                dialog.dismiss()
                executeUpdateStatus(siswa, newStatus)
            }
            .setNegativeButton("Batal", null)
            .show()
    }

    private fun executeUpdateStatus(siswa: SiswaItem, newStatus: String) {
        val token = getAuthToken()
        if (token.isEmpty()) return

        lifecycleScope.launch {
            val request = com.xirpl2.SASMobile.model.UpdateStatusRequest(status = newStatus)
            repository.updateStudentStatus(token, siswa.nis, request).fold(
                onSuccess = {
                    runOnUiThread {
                        Toast.makeText(this@DataSiswaAdminActivity, "Status ${siswa.nama_siswa} berhasil diubah", Toast.LENGTH_SHORT).show()
                        loadStudentData(reset = true)
                    }
                },
                onFailure = { error ->
